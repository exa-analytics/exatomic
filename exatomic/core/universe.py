...
def to_cube(uni,file_name='output',field_number=0)
    ...
    f_out.write('OUTER LOOP: X, MIDDLE LOOP: Y, INNER LOOP: Z\n')
    f_out.write('{:4}{:12.6f}{:12.6f}{:12.6f}\n'.format(len(uni.atom),
                                                 uni.field['ox'][n], uni.field['oy'][n], 
                                                 uni.field['oz'][n]))
    NX = uni.field['nx'][n]
    f_out.write('{:4}{:12.6f}{:12.6f}{:12.6f}\n'.format(NX,
                                                 uni.field['dxi'][n], uni.field['dxj'][n], 
                                                 uni.field['dxk'][n]))
    NY = uni.field['ny'][n]
    f_out.write('{:4}{:12.6f}{:12.6f}{:12.6f}\n'.format(NY,
                                                 uni.field['dyi'][n], uni.field['dyj'][n], 
                                                 uni.field['dyk'][n]))
    NZ = uni.field['nz'][n]
    f_out.write('{:4}{:12.6f}{:12.6f}{:12.6f}\n'.format(NZ,
                                                 uni.field['dzi'][n], uni.field['dzj'][n], 
                                                 uni.field['dzk'][n]))
    for index, row in uni.atom.iterrows():
        a = row['Z']
        #Nuclear charge on atom a, this ought to deviate from the atomic number such as when
        #an ECP is used, although I couldn't find an entry for this so assume a is equal to
        #atomic number Z
        f_out.write('{:4}{:12.6f}{:12.6f}{:12.6f}{:12.6f}\n'.format(row['Z'],
                                                                    a, row['x'],
                                                                    row['y'], row['z']))
    for i in range(NX):
        for j in range(NY):
            for k in range(NZ):
                f_out.write(f'{uni.field.field_values[n][i*NY*NZ+j*NZ+k]:13E} ')
                if k%6 == 5:
                    f_out.write('\n')
            f_out.write('\n')
    f_out.close()
