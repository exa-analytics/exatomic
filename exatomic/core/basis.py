...
"""
Basis Set Data Table
##############################
This module provides the :class:`~exatomic.core.basis.BasisSet` and related
classes which describe various types of basis sets common to quantum chemistry.
...
str2l = {'s': 0, 'p': 1, 'd': 2, 'f': 3, 'g': 4, 'h': 5, 'i': 6}
_letts = ascii_lowercase[10:].replace("s", "").replace("p", "")
str2l.update({v: k for k, v in dict(enumerate(_letts, start=7)).items()})
...
class GaussianBasisSet(BasisSet):
    ...
    This table stores the most basic information about a Gaussian basis, the
    primitive (Cartesian/spherical) Gaussian functions. These functions have
    the following form.

    .. math::

        G_{ijk}\left(\mathbf{r}, a, \mathbf{A}\right) = x_A^iy_A^jz_A^ke^{-ar_A^2}

    The angular momentum of a primitive function is defined by $l = i + j + k$.
    Each primitive is centered on a particular atom, $\mathbf{A}$. The subscripts
    ...
    Attributes:
        ...
        d (float): Contraction coefficient (1.0 for non-contracted functions)
        spherical (bool): Spherical basis (true) or Cartesian (false)
    """
    ...
    def expand(self):
        """
        Expand the concise form of the basis in terms of spin angular momentum.
        """
        pass

    def __init__(self, *args, **kwargs):
        order = kwargs.pop("order", None)
        spherical = kwargs.pop("spherical", None)
        if order is None or spherical is None:
            raise ValueError("Required kwargs 'order'/'spherical' missing.")
