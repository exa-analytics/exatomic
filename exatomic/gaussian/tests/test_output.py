...
class TestOutput(TestCase):
    ...
    def test_parse_atom(self):
        ...
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.uo2.atom))).all())
        self.mam3.parse_atom()
        self.assertEqual(self.mam3.atom.shape[0], 7)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam3.atom))).all())
        self.mam4.parse_atom()
        self.assertEqual(self.mam4.atom.shape[0], 7)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam4.atom))).all())
        self.meth_opt.parse_atom()
        self.assertEqual(self.meth_opt.atom.shape[0], 120)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.meth_opt.atom))).all())
        self.nap_opt.parse_atom()
        self.assertEqual(self.nap_opt.atom.shape[0], 806)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.nap_opt.atom))).all())
        self.meth_opt_freq_hp.parse_atom()
        self.assertEqual(self.meth_opt_freq_hp.atom.shape[0], 130)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.meth_opt_freq_hp.atom))).all())

    def test_parse_basis_set(self):
        self.uo2.parse_basis_set()
        self.assertEqual(self.uo2.basis_set.shape[0], 49)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.uo2.basis_set))).all())
        ...
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam4.basis_set))).all())

    def test_parse_orbital(self):
        self.uo2.parse_orbital()
        self.assertEqual(self.uo2.orbital.shape[0], 141)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.uo2.orbital))).all())
        self.mam3.parse_orbital()
        self.assertEqual(self.mam3.orbital.shape[0], 28)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam3.orbital))).all())
        self.mam4.parse_orbital()
        self.assertEqual(self.mam4.orbital.shape[0], 91)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam4.orbital))).all())
        self.meth_opt.parse_orbital()
        self.assertEqual(self.meth_opt.orbital.shape[0], 160)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.meth_opt.orbital))).all())
        self.nap_tddft.parse_orbital()
        self.assertEqual(self.nap_tddft.orbital.shape[0], 611)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.nap_tddft.orbital))).all())

    def test_parse_momatrix(self):
        self.uo2.parse_momatrix()
        self.assertEqual(self.uo2.momatrix.shape[0], 19881)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.uo2.momatrix))).all())
        self.mam3.parse_momatrix()
        self.assertEqual(self.mam3.momatrix.shape[0], 784)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam3.momatrix))).all())
        self.mam4.parse_momatrix()
        self.assertEqual(self.mam4.momatrix.shape[0], 8281)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam4.momatrix))).all())

    def test_parse_basis_set_order(self):
        self.uo2.parse_basis_set_order()
        self.assertEqual(self.uo2.basis_set_order.shape[0], 141)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.uo2.basis_set_order))).all())
        self.mam3.parse_basis_set_order()
        self.assertEqual(self.mam3.basis_set_order.shape[0], 28)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam3.basis_set_order))).all())
        self.mam4.parse_basis_set_order()
        self.assertEqual(self.mam4.basis_set_order.shape[0], 91)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam4.basis_set_order))).all())

    def test_parse_frame(self):
        self.uo2.parse_frame()
        self.assertEqual(self.uo2.frame.shape[0], 1)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.uo2.frame))).all())
        self.mam3.parse_frame()
        self.assertEqual(self.mam3.frame.shape[0], 1)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam3.frame))).all())
        self.mam4.parse_frame()
        self.assertEqual(self.mam4.frame.shape[0], 1)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.mam4.frame))).all())
        self.meth_opt.parse_frame()
        self.assertEqual(self.meth_opt.frame.shape[0], 12)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.meth_opt.frame))).all())
        self.nap_opt.parse_frame()
        self.assertEqual(self.nap_opt.frame.shape[0], 26)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.nap_opt.frame))).all())
    ...
    def test_parse_frequency_ext(self):
        ...
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.meth_freq.frequency_ext))).all())
        ...
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.meth_opt_freq_hp.frequency_ext))).all())

    def test_parse_frequency(self):
        self.meth_freq.parse_frequency()
        self.assertEqual(self.meth_freq.frequency.shape[0], 240)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.meth_freq.frequency))).all())
        self.meth_opt_freq_hp.parse_frequency()
        self.assertEqual(self.meth_opt_freq_hp.frequency.shape[0], 240)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.meth_opt_freq_hp.frequency))).all())

    def test_parse_excitation(self):
        self.nap_tddft.parse_excitation()
        self.assertEqual(self.nap_tddft.excitation.shape[0], 15)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.nap_tddft.excitation))).all())
        self.h2o2_tddft.parse_excitation()
        self.assertEqual(self.h2o2_tddft.excitation.shape[0], 32)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.h2o2_tddft.excitation))).all())
    ...
    def test_parse_gradient(self):
        ...
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.meth_opt.gradient))).all())
        self.nap_opt.parse_gradient()
        self.assertEqual(self.nap_opt.gradient.shape[0], 806)
        self.assertTrue(np.all(pd.notnull(pd.DataFrame(self.nap_opt.gradient))).all())
